    ...
    for index, row in df.iterrows():
        ...
        if pd.isna(key) or key == "":
            if comments == "root object added":
                new_root_object = service_name  # Use the service name as the new root object
                if new_root_object not in json_data:
                    json_data[new_root_object] = {}  # Initialize the new root object
                    print(f"Added new root object: {new_root_object}")

                # Store the value directly under the new root object
                if not pd.isna(value) and value != "":
                    json_data[new_root_object] = try_parse_json(value)  # Directly assign the parsed value to the root object
